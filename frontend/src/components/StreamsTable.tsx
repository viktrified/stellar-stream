...
function FilterBar({
  ...
  const hasActiveFilters = !!(filters.status || filters.asset || filters.sender);

  return (
    <div className="filter-bar" role="search" aria-label="Filter streams">
      <select
      ...
      <input
        ...
        maxLength={12}
        onChange={(e) =>
          onChange({ ...filters, asset: e.target.value.trim() || undefined })
        }
      />

      <input
        type="text"
        aria-label="Filter by sender address"
        placeholder="Sender address"
        value={filters.sender ?? ""}
        maxLength={64}
        onChange={(e) =>
          onChange({ ...filters, sender: e.target.value.trim() || undefined })
        }
      />
...
export function StreamsTable({
  ...
  function toggleTimeline(streamId: string) {
